...
export const getVigilanteStatus = (
  vig: Vigilante,
  day: number,
  timeStr: string,
) => {
  ...
  if (!timeStr)
    return {
      ...
      status: "NO POSTO",
      variant: "success",
      location: coversToday ? coversToday.local : vig.campus,
    };
  ...
  if (!workRange.crossDay) {
    isWorkingHour =
      targetMins >= workRange.start && targetMins <= workRange.end;
  } else {
    isWorkingHour =
      targetMins >= workRange.start || targetMins <= workRange.end;
  }
...
export const calculateIntervalRisk = (
  sector: string,
  refeicaoStr: string,
  ...
  const isCriticalSector =
    sectorUpper.includes("PORTARIA") ||
    sectorUpper.includes("CATRACA") ||
    sectorUpper.includes("GUARITA");
  const isClassBlock =
    sectorUpper.includes("BLOCO") ||
    sectorUpper.includes("SALA") ||
    sectorUpper.includes("ALFA") ||
    sectorUpper.includes("CHARLIE");
  ...
  const overlapsMorning = overlap(
    range.start,
    range.end,
    PEAK_HOURS.MORNING.start,
    PEAK_HOURS.MORNING.end,
  );
  const overlapsLunch = overlap(
    range.start,
    range.end,
    PEAK_HOURS.LUNCH.start,
    PEAK_HOURS.LUNCH.end,
  );
  const overlapsEvening = overlap(
    range.start,
    range.end,
    PEAK_HOURS.EVENING.start,
    PEAK_HOURS.EVENING.end,
  );
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    Object.keys(map).forEach((campus) => {
      Object.keys(map[campus]).forEach((eq) => {
        if (
          targetTeam !== "TODAS" &&
          cleanString(eq) !== cleanString(targetTeam)
        )
          return;
        ...
        if (standardDays.includes(day)) {
          ...
          teamMembers.forEach((v) => {
            ...
            const isVacation =
              v.vacation && day >= v.vacation.start && day <= v.vacation.end;
          ...
          if (
            totalMembers > 0 &&
            workingCount < Math.ceil(totalMembers * 0.5)
          ) {
            conflicts.push({
              dia: day,
              campus,
