...
import { Vigilante, AuthResponse, AuditLog, User } from '../types';
...
export const api = {
    ...
    updateConnection(url: string, key: string) { return true; },

    isConfigured() { return !!supabase; },

    async getSystemStatus(): Promise<{ online: boolean; message: string; code?: string }> {
        if (!supabase) return { online: false, message: 'Cliente Supabase não inicializado.' };
        try {
            ...
            if (error) {
                ...
                if (error.code === '42P01') return { online: false, message: 'Tabela "escalas" não encontrada no Supabase.', code: error.code };
                if (error.message && error.message.includes('FetchError')) return { online: false, message: 'Sem internet.', code: 'NET_ERR' };
                return { online: false, message: `Erro API: ${error.message}`, code: error.code };
            }
            return { online: true, message: 'Conectado à Nuvem (Supabase).' };
        } catch (e: any) {
            return { online: false, message: `Falha de Rede: ${e.message || 'Desconhecida'}` };
        }
    },
    ...
    async login(mat: string, password: string): Promise<AuthResponse> {
        if (!supabase) return { success: false, message: "Erro crítico: DB Offline." };

        try {
            const { data, error } = await supabase.from(TABLE).select('dados').eq('nome', 'users').single();
            ...
            if (error) return { success: false, message: "Erro ao conectar ou usuário não encontrado." };
    ...
    async getUsers(): Promise<User[]> {
        if (!supabase) return [];
        try {
            const { data } = await supabase.from(TABLE).select('dados').eq('nome', 'users').single();
            return data?.dados || [];
        } catch (e) { return []; }
    },
    ...
    async saveUsers(users: User[]): Promise<boolean> {
        ...
        try {
            const { error } = await supabase.from(TABLE).upsert({ nome: 'users', dados: users }, { onConflict: 'nome' });
            return !error;
    ...
    async seedUsers(vigilantes: Vigilante[]): Promise<void> {
        ...
        try {
            ...
            vigilantes.forEach(vig => {
                if (vig.mat !== adminMat && !currentUsers.find(u => u.mat === vig.mat)) {
                     currentUsers.push({
                        mat: vig.mat,
                        nome: vig.nome,
            ...
            if (hasChanges) {
                await supabase.from(TABLE).upsert({ nome: 'users', dados: currentUsers }, { onConflict: 'nome' });
            }
        } catch (e) { console.error("Erro Seed:", e); }
    },
    ...
    async loadData(month: number, isDraft = false): Promise<Vigilante[] | null> {
        ...
        try {
            ...
            const { data, error } = await supabase.from(TABLE).select('dados').eq('nome', key).single();
            if (error) return null;
            return data?.dados || null;
        } catch (e) { return null; }
    },

    async saveData(month: number, data: Vigilante[], isDraft = false): Promise<boolean> {
        if (!supabase) return false;
        try {
            let key = `unoeste_pro_${month}`;
            if (isDraft) key += '_draft';
            
            const { error } = await supabase.from(TABLE).upsert({ nome: key, dados: data }, { onConflict: 'nome' });
            if (error) {
    ...
    async loadLogs(month: number): Promise<AuditLog[]> {
        ...
        try {
            ...
            const { data } = await supabase.from(TABLE).select('dados').eq('nome', key).single();
            return data?.dados || [];
        } catch (e) { return []; }
    },
    ...
    async addLog(month: number, log: AuditLog): Promise<void> {
        ...
        try {
            ...
            const key = `unoeste_logs_${month}`;
            await supabase.from(TABLE).upsert({ nome: key, dados: newLogs }, { onConflict: 'nome' });
