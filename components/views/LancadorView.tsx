...
interface LancadorViewProps {
    ...
    setTimeInputs: (v: { hStart: string; hEnd: string; rStart: string; rEnd: string }) => void;
...
const LancadorViewComponent: React.FC<LancadorViewProps> = (props) => {
    const { 
        ...
        handleSaveEditor, handleDeleteVigilante, handleToggleDay, handleToggleVacation, handleToggleFalta, handleTogglePartial,
    ...
    return (
        <div className="flex flex-1 h-full overflow-hidden bg-slate-900 relative print:h-auto print:overflow-visible">
            <div className={`w-full md:w-[380px] bg-slate-800 border-r border-slate-700 flex flex-col shadow-xl z-20 shrink-0 h-full absolute md:relative top-0 left-0 bottom-0 transition-transform duration-300 ease-in-out ${showMobileEditor ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} print:hidden`}>
                ...
                <div className="p-4 bg-slate-900 border-b border-slate-700 space-y-3 shrink-0">
                    {user?.role !== 'FISCAL' && (
                        <div>
                            <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">1. Filtrar Equipe:</label>
                            <Select value={selectedLancadorTeam} onChange={(e) => setSelectedLancadorTeam(e.target.value)} className="bg-slate-700 text-white border-slate-600 shadow-sm">
                    ...
                    <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">2. Buscar Nome:</label>
                        <Input placeholder="Digite para filtrar a lista..." value={lancadorSearch} onChange={(e) => setLancadorSearch(e.target.value)} className="bg-slate-700 text-white border-slate-600 shadow-sm" />
                    </div>
                </div>
                    ...
                    {editingVig ? (
                        <div className="bg-slate-800 rounded-xl shadow-md border border-slate-600 overflow-hidden animate-fade-in">
                            <div className="bg-slate-800 p-4 border-b border-slate-700 text-center">
                                <h3 className="font-bold text-lg text-white leading-tight">{editingVig.nome}</h3>
                                <div className="text-xs text-slate-400 mt-1">{editingVig.mat} | Eq <Badge team={editingVig.eq} /></div>
                            ...
                            <div className="p-4 space-y-4">
                                <div>
                                    <label className="text-[10px] font-bold text-slate-300 block mb-1">Setor:</label>
                                    <Input list="sector-options" value={editingVig.setor} onChange={(e) => setEditingVig({...editingVig, setor: e.target.value.toUpperCase()})} className="h-8 text-xs bg-slate-700 text-white border-slate-600" />
                                    <datalist id="sector-options">{SECTOR_OPTIONS.map(s => <option key={s} value={s} />)}</datalist>
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-slate-300 block mb-1">Horário:</label>
                                    <div className="flex items-center gap-2">
                                        <input type="time" className="flex-1 border border-slate-600 bg-slate-700 text-white rounded p-1.5 text-xs text-center font-bold" value={timeInputs.hStart} onChange={e => setTimeInputs({...timeInputs, hStart: e.target.value})} />
                                        <span className="text-[10px] font-bold text-slate-400">às</span>
                                        <input type="time" className="flex-1 border border-slate-600 bg-slate-700 text-white rounded p-1.5 text-xs text-center font-bold" value={timeInputs.hEnd} onChange={e => setTimeInputs({...timeInputs, hEnd: e.target.value})} />
                                    </div>
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-slate-300 block mb-1">Refeição:</label>
                                    <div className="flex items-center gap-2">
                                        <input type="time" className="flex-1 border border-slate-600 bg-slate-700 text-white rounded p-1.5 text-xs text-center font-bold" value={timeInputs.rStart} onChange={e => setTimeInputs({...timeInputs, rStart: e.target.value})} />
                                        <span className="text-[10px] font-bold text-slate-400">às</span>
                                        <input type="time" className="flex-1 border border-slate-600 bg-slate-700 text-white rounded p-1.5 text-xs text-center font-bold" value={timeInputs.rEnd} onChange={e => setTimeInputs({...timeInputs, rEnd: e.target.value})} />
                                    </div>
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-slate-300 block mb-1">Campus:</label>
                                    <select className="w-full text-xs border border-slate-600 rounded p-1.5 bg-slate-700 text-white" value={editingVig.campus} onChange={(e) => setEditingVig({...editingVig, campus: e.target.value})}>
                                ...
                                <div className="border-t border-slate-700 pt-2 pb-1">
                                    <div className="flex bg-slate-900 rounded-lg p-1 gap-1">
                                        <button onClick={() => setEditorMode('days')} className={`flex-1 py-1.5 text-[10px] font-bold rounded-md transition-all ${editorMode === 'days' ? 'bg-slate-700 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>📅 DIAS</button>
                                        ...
                                        <button onClick={() => setEditorMode('falta')} className={`flex-1 py-1.5 text-[10px] font-bold rounded-md transition-all ${editorMode === 'falta' ? 'bg-red-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>❌ FALTA</button>
                                        <button onClick={() => setEditorMode('partial')} className={`flex-1 py-1.5 text-[10px] font-bold rounded-md transition-all ${editorMode === 'partial' ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>⚠️ PARCIAL</button>
                                    </div>
                                </div>
                                <CalendarGrid
                                    vig={editingVig}
                                ...
                                <div className="flex flex-col gap-2 pt-2 border-t border-slate-700">
                                    ...
                                    <Button onClick={handleDeleteVigilante} className="w-full h-8 text-xs bg-red-900/30 hover:bg-red-900/50 text-red-400 border border-red-800 font-bold">EXCLUIR VIGILANTE</Button>
                                </div>
                            </div>
                        </div>
                    ) : (
            ...
            <div className="flex-1 flex flex-col bg-slate-900 overflow-hidden h-full relative z-10 w-full print:overflow-visible print:h-auto">
                <div className="flex items-center gap-4 p-3 bg-slate-900 border-b border-slate-700 shadow-sm shrink-0 print:hidden">
                    <div className="flex-1 flex gap-2 overflow-x-auto no-scrollbar">
                        <div className="bg-slate-800 px-3 py-1 rounded text-xs font-bold text-slate-400 border border-slate-700 whitespace-nowrap">Total {lancadorSummary.total}</div>
                        ...
                        <table className="w-full text-left text-xs">
                            ...
                            <tbody className="divide-y divide-slate-700">
                                {lancadorList.map((vig) => {
                                    ...
                                    const hasAlert = (vig.dias || []).length === 0 || ((vig.folgasGeradas || []).length > 0 && !vig.manualLock);
                                    
                                    return (
                                    <tr key={vig.mat} onClick={() => setEditingVig(vig)} className={`cursor-pointer transition-colors ${editingVig?.mat === vig.mat ? 'bg-blue-900/30 border-l-4 border-l-blue-500' : 'hover:bg-slate-700 even:bg-slate-800/50'} ${vig.manualLock ? 'text-slate-200' : 'bg-orange-900/20 text-orange-200'}`}>
                                        <td className="px-4 py-3 font-bold">
                                            {vig.manualLock ? (
                                                <span className="flex items-center gap-1 text-slate-400"><span className="text-lg">👤</span> OK</span>
                                            ) : (
                                        ...
                                        <td className="px-4 py-3 font-bold flex items-center gap-2">
                                            {vig.nome}
                                            {hasAlert && <span className="text-lg animate-pulse" title="Alerta: Sem escala ou folgas pendentes">⚠️</span>}
                                        </td>
