...
export const AlocacaoView: React.FC<AlocacaoViewProps> = ({
    ...
    // Helper for smart sector matching (ID vs Label vs Preset Name)
    const isSectorMatch = (vigSector: string, presetSector: string, presetName?: string) => {
        const v = cleanString(vigSector);
        const p = cleanString(presetSector);

        // Direct match (Sector ID)
        if (v === p) return true;

        // Direct match (Preset Name - Crucial for Custom/Duplicate Presets)
        if (presetName && v === cleanString(presetName)) return true;

        // Loose match (ignore dashes/spaces)
        if (v.replace(/[^A-Z0-9]/g, "") === p.replace(/[^A-Z0-9]/g, "")) return true;

        // Resolve Fixed Sector ID (vig uses ID, preset uses Label)
        const fixedById = FIXED_SECTORS.find(fs => fs.id === vigSector);
        if (fixedById && cleanString(fixedById.label) === p) return true;
    ...
    const groupedPresets = useMemo(() => {
        ...
        // DEBUG: Validate Master Access
        if (isMaster) {
            console.log("DEBUG: Master Access Check", {
                lancadorVisibleTeams,
        ...
        presets.forEach((p) => {
            ...
            if (p.team) {
                const requiredTeam = cleanString(p.team).replace(/\s+/g, "");
                const userCanSee = lancadorVisibleTeams
                    .map(t => cleanString(t).replace(/\s+/g, ""))
            ...
            if (filterTeam !== "TODAS") {
                ...
                // RE-FIX: Strict Team Check (Handles logic for ECO vs A/B)
                // Only apply if preset has a specific team defined (e.g. "ECO", "ADM")
                if (match && p.team) {
                    const pTeamNorm = cleanString(p.team).replace(/\s+/g, ""); // "ECO"
                    const tTargetNorm = cleanString(target).replace(/\s+/g, ""); // "A"

                    // If preset matches "ECO", allow "ECO1"/"ECO2"/"ECO" filters.
                    if (pTeamNorm === "ECO") {
                        const actsAsEco = tTargetNorm.includes("ECO") || tTargetNorm.startsWith("E");
                        if (!actsAsEco) match = false; // Hide "ECO" sector from "A" filter
                    }
                    // Generic strict match
                    else if (pTeamNorm !== tTargetNorm) {
                        match = false;
                    }
                }

                // STRICT PRESET TEAM CHECK
                // If the preset belongs to a specific team (e.g. "ECO"), HIDE IT if filtering for "A".
                if (p.team) {
                    const pTeam = cleanString(p.team).replace(/\s+/g, "");
                    const tTarget = target.replace(/\s+/g, "");

                    // Normalize ECO
                    const normP = (pTeam === "E1" || pTeam === "ECO1") ? "ECO1" : (pTeam === "E2" || pTeam === "ECO2") ? "ECO2" : pTeam;
                    const normT = (tTarget === "E1" || tTarget === "ECO1") ? "ECO1" : (tTarget === "E2" || tTarget === "ECO2") ? "ECO2" : tTarget;

                    // If preset matches target, great. If not, it MUST be hidden.
                    // Exception: If pTeam is "ECO" (generic), allow ECO1/ECO2? 
                    // Currently FIXED_SECTORS use "ECO" (generic).
                    if (normP === "ECO") {
                        if (normT !== "ECO1" && normT !== "ECO2" && normT !== "ECO") match = false;
                    } else {
                        if (normP !== normT) match = false;
                    }
                }
            ...
            // DEBUG: Trace ALFA filtering
            if (sector.includes("ALFA") || sector.includes("FAQUATRO")) {
                console.log(`DEBUG: Filter Trace for ${p.name} (${p.id})`, {
                    userVisible: lancadorVisibleTeams,
                    requiredTeam: p.team,
                    type: p.type,
                    isStrictDiurno,
                    isStrictNoturno,
                    shouldShow,
                    filterTeam,
                    rejectionReason: !shouldShow ? "Rejected by logic" : "Allowed"
                });
            }
    ...
    const handleAssign = (preset: DepartmentPreset, mat: string) => {
        onUpdateVigilante(mat, {
            ...
            setor: preset.name, // Link to Preset Name (e.g. "ALFA 4") not raw sector
                                            ...
                                            const allOccupants = vigilantes
                                                .filter(
                                                    (v) =>
                                                        ...
                                                        isSectorMatch(v.setor, preset.sector, preset.name),
                                                )
                                                .sort((a, b) => a.nome.localeCompare(b.nome));

                                            // Prioritize occupants matching the preset's team
                                            // TIER 1: EXACT MATCH BY PRESET NAME (Explicit Assignment)
                                            let availableOccupant = allOccupants.find(v =>
                                                !displayedVigilantes.has(v.mat) &&
                                                cleanString(v.setor) === cleanString(preset.name)
                                            );

                                            // TIER 2: STRICT TEAM MATCH (Using Sector ID)
                                            if (!availableOccupant) {
                                                availableOccupant = allOccupants.find((v) => {
                                                    if (displayedVigilantes.has(v.mat)) return false;
                                                    if (!preset.team) return true;

                                                    // Check Team Match
                                                    const vTeam = cleanString(v.eq).replace(/\s+/g, "");
                                                    const pTeam = cleanString(preset.team).replace(/\s+/g, "");

                                                    if (vTeam === pTeam) return true;
                                                    if ((vTeam === "E1" || vTeam === "ECO1") && (pTeam === "E1" || pTeam === "ECO1")) return true;
                                                    if ((vTeam === "E2" || vTeam === "ECO2") && (pTeam === "E2" || pTeam === "ECO2")) return true;

                                                    return false;
                                                });
                                            }

                                            // TIER 3: FALLBACK (Any remaining match in sector)
                                            // This fixes "Vago" when a Team A person is covering a Team D slot,
                                            // OR when the team metadata is missing/mismatched but ID matches.
                                            ...
                                            const filteredOptions = filteredVigilantes
                                                .filter((v) => {
                                                    ...
                                                    const isAlreadyHere =
                                                        ...
                                                        isSectorMatch(v.setor, preset.sector);
