...
async function main() {
  console.log("Probing USUARIOS permission columns...");
  await checkColumn("canManageIntervals");
  await checkColumn("canViewLogs");
  await checkColumn("canPrint");
  await checkColumn("canSimulate");
  await checkColumn("canViewCFTV");
  // Also check snake_case versions just in case
  await checkColumn("can_manage_intervals");
  await checkColumn("can_view_logs");
}
